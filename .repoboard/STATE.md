...
**Written 2026-09-18T20:34:10Z by coordinator.**
...
- **repoboard builder (its own terminal): UP, RCB-47 LANDED 00aae0a 20:31Z — verified by content on origin/main by the coordinator 20:4xZ** (core `lastBlockFor` + store `lastRepoLogBlock` + CLI `log --last <seat>` + docs/AGENTS.md cold-start rule; tried live after `pnpm build`: `log --last builder` returns its 20:31Z block, an unknown seat prints `(no log block for …)` exit 0). Gate 639+2 skipped ×2, typecheck/lint/build 0; card → done by the coordinator; the builder releases its own RCB-47 lease on this word. **Now RCB-52** (owner tasks in the one owner queue) as `decision.kind: task` on the EXISTING block — coordinator CONCURS (O10, `needsDecision` stays the single gate; the `owner:` word is a rendering); brief docs/RCB-52-OWNER-TASK-BRIEF.md; agent A core+server then agent B web. Queue after: RCB-48 → RCB-49 (v0.1.0 tag) → RCB-34 → K5/RCB-51 (after the owner's npm) → RCB-43. **Its session's permission classifier BLOCKS `state --set-section` (twice tonight)** — the coordinator restamps SEATS after each verified landing (its routine duty), and the owner may add a rule in that terminal if he wants the builder self-stamping; a seat never routes around it. Rig facts unchanged (19:4xZ line): `pnpm build` first; vitest lock `mkdir || exit` + owner line, never rm a foreign lock, no suite inside an fpj gate window; rebuild + restart :4242/:4243 after any WEB landing and look at :4243 in a browser; never touch :5173/:8787; `repoboard check` ok at start and stop.
