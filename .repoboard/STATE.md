...
**Written 2026-09-18T21:22:50Z by coordinator.**
...
- **repoboard builder (its own terminal): DOWN 2026-09-18 21:2xZ (250k handoff). Landed tonight, all verified by content on origin/main: RCB-47 00aae0a · RCB-52 552fb2e · RCB-53 a892b11 · RCB-48 6ddc269 · RCB-49 4c9037f (CONTRIBUTING, CODE_OF_CONDUCT, SECURITY, CHANGELOG, .github templates) · tag v0.1.0 = 5477022 on 4c9037f, pushed. All cards done, no lease held. Stand-down block = last BUILDER entry in .repoboard/log/2026-09-18.md.** FRESH BUILDER: cold-start with `node packages/server/dist/cli.js seat builder` after `pnpm build` (own board only — it cannot read fpj's docs/log yet, that is RCB-54). Queue: **RCB-54** (`log --last`/`seat` honour board.yml `logDir` + widen the heading regex for fpj's `##### OPS 2026-09-18 19:5xZ:` shape; causes named on the card) → RCB-34 (columns editable, O6) → K5/RCB-51 (after the owner's npm — Done button on RCB-51 is the signal) → RCB-43. Rig facts (19:4xZ line): `pnpm build` first; vitest lock `mkdir /tmp/fpj-vitest.lock || exit` + owner line, never rm a foreign lock, no suite inside an fpj gate window; rebuild + restart :4242 (this root) and :4243 (fpj root, `node <this repo>/packages/server/dist/cli.js serve --port 4243`) after any WEB landing and look at :4243 in a browser; never touch :5173/:8787; sonnet subagents with written briefs; one commit per card with its numbers; `repoboard check` ok at start and stop; report each sha to the coordinator, who verifies by content, moves the card and restamps SEATS (the classifier in the builder's terminal usually blocks `state --set-section`).
