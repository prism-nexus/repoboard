...
size: M  # S | M | L | XL, optional — S ≤2h · M half a day · L days · XL plan-sized
labels: [web, viz]
files:
...
parent: RB-9            # optional (RCB-68); this card is a STEP of that phase card
phase: PH.3             # optional (RCB-68); free short label, sorted naturally
gate: RB-9              # optional (RCB-68); a card id or "<sentence>" naming what blocks it
...
- `POST /api/cards/:id/notes` `{text, actor?}` → `Card` (RCB-70)
...
- `Card` gains `parent?, phase?, gate?` (RCB-68); every compact row (CLI `--json`, MCP `list_cards`) gains `parent, phase, gate, blocked` — the last computed live, never stored.
- `Card` gains `size?` (RCB-67, `S | M | L | XL`); every compact row (CLI `--json`, MCP `list_cards`) gains `size`, `null` when unset.
...
- **P8.1 Decisions ON THE CARD** (rewritten on O10, amended on O11) — **Landed `aef44c3`.** A card
  ...
  `needsDecision` stay, for an agent with no dashboard). Brief: `.repoboard/local/briefs/P8.1-DECISIONS-BRIEF.md`.
...
- **P8.3 State, the daily log, `init --practices`, `check`** — `.repoboard/STATE.md` (fixed sections
  ...
  Brief: `.repoboard/local/briefs/P8.3-STATE-LOG-BRIEF.md`. **Landed `562a09d`.**
- **P8.4 Cost** — **Landed `b16c115`.** `repoboard cost [--root <dir>] [--budget <bytes>] [--json]`:
  ...
  linked file) — `docs/REFERENCE.md` §4 has both tables. Brief: `.repoboard/local/briefs/P8.4-COST-BRIEF.md`.
- **P8.5 Archive and issue sync** — `repoboard archive [--older-than 14d]` moves `done` cards whose
  ...
  **Landed `3c314d5`** (verified by the orchestrator: 562/562 ×2 independently, typecheck 0, lint 0; fpj dry-run re-run read-only at 0427693 → 64/0/0, K list identical to an independent grep; C1 and C2 re-perturbed by the orchestrator and watched failing). Measured read-only against freshpickedjobs three times
  ...
  `.repoboard/` absent throughout. `docs/REFERENCE.md` §5 has the full table and the K-number list.
  Brief: `.repoboard/local/briefs/P8.5-ARCHIVE-SYNC-BRIEF.md` §7.
...
- **O5 — K7 pulled into v0.1 (2026-09-03).** The owner: "the repo should be looking at where all
  ...
  Brief: `.repoboard/local/briefs/K7-REFS-BRIEF.md`.
...
- **O7 — repoboard points at any repo, and the map works without a board.** The owner: "we also
  ...
  Tasks P7.1, P7.2. Brief: `.repoboard/local/briefs/P7-REPOS-BRIEF.md`.
- **O8 — K9: the CLI gets `card update <id>`, not `--assign` on `card move`.** Decided
  ...
  and it is the only new concept in the change. Brief: `.repoboard/local/briefs/K9-UPDATE-BRIEF.md`. Card: RCB-31.
