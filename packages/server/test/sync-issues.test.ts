...
describe('server: computeSyncPlan / applySyncPlan', () => {
  ...
  it("RCB-67: sync-issues preserves a card's size across a close (applySyncPlan/closeSyncedCard do not overwrite fields)", async () => {
    const root = await fixture();
    const store = await openStore(root, { watch: false, now: () => NOW });
    const input = { path: 'README.md', heading: 'Known issues', status: 'todo', label: 'issue' };
    const outcome1 = await computeSyncPlan(store, input);
    if (!outcome1.ok) throw new Error(outcome1.error);
    const applied1 = await applySyncPlan(store, input, outcome1.plan, 'claude/sync');
    expect(applied1.errors).toEqual([]);

    const k1 = store.list().find((c) => (c.refs ?? []).includes('README.md@K1'));
    if (!k1) throw new Error('K1 card not filed');
    // Through the store, not the shell (K9/plan §11 O8's own words): set size by hand, as an
    // owner would, to prove sync leaves it alone.
    const sized = await store.update(k1.id, { size: 'M' }, 'claude/owner');
    if (!sized.ok) throw new Error(sized.error);
    expect(sized.card.size).toBe('M');

    // Strike K1 in the source (never through sync — sync-issues never writes README.md).
    const struckReadme = README.replace(
      '- **K1. First open item** — more prose.',
      '- ~~**K1. First open item~~ — more prose.',
    );
    await writeFile(join(root, 'README.md'), struckReadme);

    const outcome2 = await computeSyncPlan(store, input);
    if (!outcome2.ok) throw new Error(outcome2.error);
    expect(outcome2.plan.close.some((c) => c.cardId === k1.id)).toBe(true);
    const applied2 = await applySyncPlan(store, input, outcome2.plan, 'claude/sync');
    expect(applied2.closed).toContain(k1.id);

    const reread = store.get(k1.id);
    expect(reread?.status).toBe('done');
    expect(reread?.size).toBe('M');
    const text = await readFile(join(root, '.repoboard', 'cards', `${k1.id}.md`), 'utf8');
    const lines = text.split('\n');
    const sizeLine = lines.indexOf('size: M');
    expect(sizeLine).toBeGreaterThan(-1);
    expect(lines[sizeLine + 1]).toBe('labels:');
  });
